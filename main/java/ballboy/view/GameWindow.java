...
public class GameWindow {
    ...
    private final BackgroundDrawer scoreDrawer;
    private double xViewportOffset = 0.0;
    private double yViewportOffset = 0.0;
    private int victoryTicks=0;
    private boolean beenDrawn=false;                 //used to make sure WINNER is drawn exactly once before termination
    ...
    public GameWindow(
        ...
        scoreDrawer = new ScoreDisplay(model, pane);
    ...
    private void draw() {
        ...
        //If we won the game
        if (model.isGameOver()){
            //Display WINNER and pause for 5 seconds before termination
            ((ScoreDisplay)scoreDrawer).victoryDisplay(model, pane);
            victoryTicks++;
            if (beenDrawn && victoryTicks >= 5){
                try{
